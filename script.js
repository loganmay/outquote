...
var renderContent = function() {
  ...
  wrapText(quoteCtx, quote + "\”", 100, canvas.height / 2 - 50, 800, 48);
  quoteCtx.fillText("\“", 86, canvas.height / 2 - 50);

  var image = new Image();
  image.onload = function() {
    quoteCtx.drawImage(image, canvas.width - 170, 70, 92, 70);
  };
  ...
  nameCtx.fillText(name + " | ", 100, canvas.height - 100);

  // TITLE TEXT
  var titleCtx = canvas.getContext("2d");
  titleCtx.font = "100 38px neuzeit-grotesk";
  titleCtx.fillText(title, nameCtx.measureText(name + " | ").width + 110, canvas.height - 100);
}
...
document.getElementById('saveButton').addEventListener("click", function() {
  ...
  saveAs(blob, name.replace(/ /g,'') + ".png");
